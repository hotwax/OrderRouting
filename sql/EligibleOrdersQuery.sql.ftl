...
from
  ...
  FROM
    ...
    <#if facilityGroupCondition?has_content>INNER JOIN FACILITY_GROUP_MEMBER FGM ON FGM.FACILITY_ID=OH.ORIGIN_FACILITY_ID AND FGM.FROM_DATE <= now() AND (FGM.THRU_DATE is null OR FGM.THRU_DATE > now()) AND <@buildSqlCondition 'FGM.FACILITY_GROUP_ID' facilityGroupCondition/></#if>
