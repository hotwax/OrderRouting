...
/*TODO: parameterized condition single allocation */          --  and case when fgm.FACILITY_GROUP_ID in ('WH_FAC_GROUP') then rank_by_order_at_facility_threshold='Y' end  -- NEW for sorting facility having all the items above threshold
...
/*TODO:: split alllocation with threashold */    and case when rank_by_order_at_facility='Y' then item_at_facility_at_threshold='Y' end
      ...
      /*TODO Remove*/    -- case when fgm.FACILITY_GROUP_ID in ('${(invenoryGroupFiter.get("fieldValue"))!}') then field(rank_by_order_at_facility_threshold,'Y','N')  -- NEW for sorting facility having all the items above threshold
      /*TODO Remove*/    -- when fgm.FACILITY_GROUP_ID in ('${(invenoryGroupFiter.get("fieldValue"))!}') then field(rank_by_order_at_facility,'Y','N') else field(rank_by_order_at_facility,'Y','N') end , -- make it case based on Sales Channel & Shipment methods
