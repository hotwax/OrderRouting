...
x.*
      from
          ...
          ifnull((select 'Y' from order_item oi3 where oi3.order_id=oi.ORDER_ID and oi3.order_item_seq_id=oi.order_item_seq_id and (pf.last_inventory_count-ifnull(pf.MINIMUM_STOCK,0)) > ${(brokeringSafetyStock.fieldValue)!0} and (pf.last_inventory_count-ifnull(pf.MINIMUM_STOCK,0)) >= oi3.quantity-ifnull(oi3.cancel_quantity,0) and oi3.STATUS_ID = 'ITEM_APPROVED'),'N') as item_at_facility_above_threshold
