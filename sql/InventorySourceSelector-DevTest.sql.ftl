...
(select sum(QUANTITY-ifnull(CANCEL_QUANTITY,0)) from order_item where order_id=oh.ORDER_ID and status_id in ('ITEM_APPROVED') and ship_group_seq_id = '00008' group by order_id)  as ship_group_total_qty,
...
ifnull((select 'Y' from order_item oi3 where oi3.order_id=oi.ORDER_ID and oi3.order_item_seq_id=oi.order_item_seq_id and (pf.last_inventory_count-ifnull(pf.MINIMUM_STOCK,0)) > 3 and (pf.last_inventory_count-ifnull(pf.MINIMUM_STOCK,0)) >= oi3.quantity-ifnull(oi3.cancel_quantity,0) and oi3.STATUS_ID in ('ITEM_APPROVED')),'N') as item_at_facility_at_threshold
...
/* left join facility_contact_mech fcm on fcm.facility_id=pf.facility_id and (fcm.thru_date is null or fcm.thru_date >= now()) */
...
left join (SELECT PFI.FACILITY_ID,PFI.LAST_INVENTORY_COUNT AS INV,ifnull(PFI.MINIMUM_STOCK,0) as MIN_STOCK, count(PFI.PRODUCT_ID) as PRODUCT_COUNT FROM PRODUCT_FACILITY PFI INNER JOIN ORDER_ITEM OII ON PFI.PRODUCT_ID=OII.PRODUCT_ID WHERE PFI.LAST_INVENTORY_COUNT > ifnull(PFI.MINIMUM_STOCK,0) and OII.ORDER_ID in ('2275') -- NEW
and ifnull(PFI.ALLOW_BROKERING,'Y') = 'Y' /* PFI.PRODUCT_ID in (SELECT OII.PRODUCT_ID FROM order_item_ship_group_assoc OISGAI, ORDER_ITEM OII WHERE OISGAI.ORDER_ID=OII.ORDER_ID AND OISGAI.ORDER_ITEM_SEQ_ID=OII.ORDER_ITEM_SEQ_ID and OII.STATUS_ID IN ('ITEM_APPROVED') and OISGAI.ORDER_ID in ('2275') and OISGAI.SHIP_GROUP_SEQ_ID in ('00008)')*/ group by PFI.FACILITY_ID, PFI.LAST_INVENTORY_COUNT,MINIMUM_STOCK having INV > 0 order by PRODUCT_COUNT DESC, INV DESC) inv_count on pf.facility_id = inv_count.facility_id
...
where oh.ORDER_ID='2275' and oi.SHIP_GROUP_SEQ_ID in (00008) -- oh.order_id in (?)  /* and ft.PARENT_TYPE_ID not in (ifnull(?,'')) */ and oisg.SHIP_GROUP_SEQ_ID in (?) and
...
AND fgm.FACILITY_GROUP_ID in ('WH_FAC_GROUP') -- NEW facility group ids need to be passed for the groups on which routing is expected to be performed
having meet_sla='Y' -- NEW to perform routing only when SLA is met
/*ADD: single allocation */          -- and case when fgm.FACILITY_GROUP_ID in ('WH_FAC_GROUP') then rank_by_order_at_facility_threshold='Y' end  -- NEW for sorting facility having all the items above threshold
--  and case when fgm.FACILITY_GROUP_ID in ('WH_FAC_GROUP') then rank_by_order_at_facility='Y' end  -- make it case based on Sales Channel & Shipment methods
/*ADD: split alllocation with threashold */   and case when rank_by_order_at_facility='Y' then item_at_facility_at_threshold='Y' end
...
/*field(f.FACILITY_TYPE_ID, ifnull(?,''), ifnull(?,''), ifnull(?,''), ifnull(?,'')),*/
